...
system.time({
  output<-foreach(1:it,.combine=rbind,.packages=c("pps","dplyr","isotone","mvtnorm","readr","MASS", "PLRModels","modi","lmtest","skedastic"))%dopar%{
    ...
    bp_1<-bptest(model_1)$p.value
    bp_2<-bptest(model_2)$p.value
    bp_3<-bptest(model_3)$p.value
    bp_4<-bptest(model_4)$p.value
    bp_5<-bptest(model_5)$p.value
    bp_6<-bptest(model_6)$p.value
    bp_7<-bptest(model_7)$p.value
    bp_8<-bptest(model_8)$p.value
    bp_9<-bptest(model_9)$p.value
    bp_10<-bptest(model_10)$p.value
    bp_11<-bptest(model_11)$p.value
    
    p<-0
    
    maximum=max(bp_1,bp_2,bp_3, bp_4,bp_5,bp_6,bp_7,bp_8,bp_9,bp_10,bp_11)
    if(maximum%in% c(bp_1,bp_2,bp_3, bp_4,bp_5,bp_6))
    p=1/it
    p
  }})
